...
/**
 * ConceptDetailPanel
 * In-place expanded detail view of a sub-concept node.
 * Features:
 * - Top header with breadcrumb-lite ("MERN Stack / MongoDB / Mongoose Schema") + heading + close button
 * - Orbital module ring for 6 learning modules (Theory, Video, Code, MCQs, Syntax, Project)
 * - Rich dynamic content card for the selected module
 * - Spring animation transitions between module swaps
 */
function ConceptDetailPanel({
  ...
  return (
    <div
      ref={panelRef}
      className="concept-detail-panel-root"
      role="dialog"
      aria-modal="true"
      aria-label={`${targetNode.label} Details`}
    >
      {/* 1. TOP HEADER BAR: BREADCRUMB-LITE + SUB-CONCEPT HEADING + CLOSE BUTTON */}
      ...
      {/* 2. MODULE ORBIT RING (THEORY, VIDEO, CODE, MCQS, SYNTAX, PROJECT) */}
      <div className="concept-orbit-selector-section">
        <div className="orbit-section-header">
          <span className="orbit-section-tag">LEARNING MODULE ORBIT</span>
          <span className="orbit-section-hint">Select a module to view interactive theory, quizzes, and code challenges</span>
        </div>
      ...
      {/* 3. DYNAMIC MODULE CONTENT CARD (SMOOTH SWAP ON MODULE CLICK) */}
      <div className="concept-module-content-card">
        {/* Module Sub-Header */}
